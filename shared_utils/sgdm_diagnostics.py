"""Optional update diagnostics shared by SGDM optimizer variants."""
...
def _spectral_norm(matrix):
    return torch.linalg.matrix_norm(matrix.detach().float(), ord=2).item()


class SGDMUpdateDiagnostics:
    """Measure observed matrix updates at selected optimizer steps."""
    ...
    def _matrix_scale(matrix_optimizer, parameter, base_learning_rate):
        if hasattr(matrix_optimizer, "multiplier_for"):
            return (
                matrix_optimizer.multiplier_for(parameter),
                matrix_optimizer.effective_learning_rate_for(parameter),
            )
        return 1.0, base_learning_rate
    ...
    def begin_step(self, step, eligible_named_parameters, matrix_optimizer):
        ...
        entries = []
        scheduled_base_learning_rates = set()
        for item in eligible_named_parameters:
            ...
            try:
                ...
                raise ValueError(
                    f"eligible matrix {item.name!r} is not owned by the matrix optimizer"
                ) from exc
            base_learning_rate = group["lr"]
            scheduled_base_learning_rates.add(base_learning_rate)
            multiplier, effective_learning_rate = self._matrix_scale(
                matrix_optimizer, parameter, base_learning_rate
            )
            entries.append(
                {
                    "name": item.name,
                    ...
                    "scheduled_learning_rate": base_learning_rate,
                    "matrix_multiplier": multiplier,
                    "effective_matrix_learning_rate": effective_learning_rate,
                    "weight_decay": group["weight_decay"],
                }
            )

        if len(scheduled_base_learning_rates) != 1:
            raise ValueError(
                "all eligible matrices must use the same scheduled base learning rate"
            )
    ...
    def end_step(self, context, matrix_optimizer, optimizer_step_applied=True):
        ...
        for entry in context["entries"]:
            ...
            record = {
                ...
                "matrix_multiplier": entry["matrix_multiplier"],
                "effective_matrix_learning_rate": entry[
                    "effective_matrix_learning_rate"
                ],
